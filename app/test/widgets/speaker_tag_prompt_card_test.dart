...
void main() {
    ...
    final provider = SpeakerTagPromptsProvider(
      ...
      submitAnswer: (request) async {
        ...
        return const ApiSuccess(
          GeneratedSpeakerTagPromptAnswerResponse(qualityOutcome: 'skipped'),
        );
      },
      updateSettings: ({
      ...
      MultiProvider(
        providers: [
          ChangeNotifierProvider(
            create: (_) => PeopleProvider(loadPeople: () async => []),
          ),
        ...
        child: const MaterialApp(
          ...
          home: Scaffold(
            body: SingleChildScrollView(child: SpeakerTagPromptCard()),
          ),
        ),
      ),
    );
  ...
  testWidgets(
    'walks through owner and confirm questions, then thanks the user',
    (tester) async {
      final (answers, _) = await pumpCard(
        tester,
        prompts: [prompt('a', 'owner_check'), prompt('b', 'confirm_person')],
      );
      expect(find.text('Is this you?'), findsOneWidget);
      expect(
        find.textContaining('We should ship it on Friday'),
        findsOneWidget,
      );
      expect(
        find.byKey(const Key('speaker_tag_prompt_save_voices_switch')),
        findsNothing,
      );
  ...
  testWidgets('clip 404 advances to the next prompt without a dead card', (
    tester,
  ) async {
    final provider = SpeakerTagPromptsProvider(
      fetchPrompts: () async => ApiSuccess(
    ...
    await tester.pumpAndSettle();
    expect(find.text('Is this Sam?'), findsOneWidget);
    expect(find.text("Couldn't play this clip"), findsNothing);
    expect(provider.current?.id, 'b');
    expect(provider.answeredCount, 0);
  });

  testWidgets(
    'first set shows the save-voices switch and a new name can be typed',
    (tester) async {
      final (answers, saves) = await pumpCard(
        tester,
        prompts: [prompt('a', 'identify')],
        firstTime: true,
      );
      final toggle = find.byKey(
        const Key('speaker_tag_prompt_save_voices_switch'),
      );
      ...
      final someoneNew = find.byKey(
        const Key('speaker_tag_prompt_answer_someone_new'),
      );
      ...
      await tester.pumpAndSettle();
      expect(
        find.byKey(const Key('speaker_tag_prompt_name_field')),
        findsOneWidget,
      );
      ...
      final save = tester.widget<TextButton>(
        find.byKey(const Key('speaker_tag_prompt_name_save')),
      );
  ...
  testWidgets('someone new opens a working name field in the iOS dialog', (
    tester,
  ) async {
    ...
    try {
      ...
      await tester.tap(
        find.byKey(const Key('speaker_tag_prompt_answer_someone_new')),
      );
      await tester.pumpAndSettle();
  ...
  testWidgets(
    'suggested people appear once the people list loads after the card',
    (tester) async {
      ...
      await tester.pumpAndSettle();
      expect(
        find.byKey(const Key('speaker_tag_prompt_answer_person_p2')),
        findsNothing,
      );
